...
class Q3_Network:
    def __init__(self, seed, dim_obs, dim_act, act_size, act_scale, batch_size, scale, ls_1, ls_2, lr_Q, lr_M, lr_S, std, SAPM):
        ...
        self.lr_M = lr_M
        self.lr_S = lr_S
        self.N_HIDDEN_1 = ls_1
        self.N_HIDDEN_2 = ls_2
    ...
    def create_net(self):
        ...
        with tf.variable_scope('q_critic'):
            ...
            h3_plus = tf.layers.dense(tf.nn.relu(h2+h3), N_HIDDEN_2, tf.nn.relu,
                                  kernel_initializer=tf.random_normal_initializer(stddev=0.01,seed=self.seed), name='H4')
            self.q_predict = tf.layers.dense(h3_plus, 1,
            ...
            h4_plus = tf.layers.dense(tf.nn.relu(h2+h4), N_HIDDEN_2, tf.nn.relu, reuse=True, name='H4')
            self.q_train = tf.layers.dense(h4_plus, 1, reuse=True, name='critic')
            ...
            stateXaction_plus = tf.layers.dense(tf.nn.relu(stateXaction), N_HIDDEN_2, tf.nn.relu, reuse=True, name='H4')
            self.stateXaction = tf.layers.dense(stateXaction_plus, 1, reuse=True, name='critic')
        ...
        with tf.variable_scope('q_target'):
            ...
            h3_plus_t = tf.layers.dense(tf.nn.relu(h2_t+h3_t), N_HIDDEN_2, tf.nn.relu,
                                  kernel_initializer=tf.random_normal_initializer(stddev=0.01,seed=self.seed), name='H4_t')
            self.q_target = tf.layers.dense(h3_plus_t, 1,
            ...
            stateXaction_plus_t = tf.layers.dense(tf.nn.relu(stateXaction_t), N_HIDDEN_2, tf.nn.relu, reuse=True, name='H4_t')
            self.stateXaction_t = tf.layers.dense(stateXaction_plus_t, 1, reuse=True, name='target')
